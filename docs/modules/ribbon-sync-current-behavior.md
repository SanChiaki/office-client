...
`ISDP AI` 组中的任务窗格按钮显示为 `Open`。`文档` 会用默认浏览器打开 `https://github.com/SanChiaki/OfficeAgent`；`关于` 会显示当前插件版本号、程序集版本、构建配置和构建时间。
...
- 如果底层业务接口返回 `401/403`，不会继续显示普通错误，而是弹出 `当前未登录，请先登录` 提示框
- 用户点击 `点我登录` 后会触发 Ribbon 登录流程
- 本次 `部分下载` 不会自动重试；登录成功后需要用户重新触发
