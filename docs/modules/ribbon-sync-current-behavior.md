...
- 但当前 Ribbon 已隐藏 `全量上传` 按钮，不再对用户直接显示
...
当前 `AI_Setting` 的展示布局是单个 sheet 内上下两个可读区域：
...
- `SheetBindings` 和 `SheetFieldMappings` 在当前活动工作簿内按表级做内存缓存，写入后同步刷新缓存
- 当用户在同一个 Excel 进程里切换到另一个工作簿时，插件会自动失效上一工作簿的 metadata 缓存，避免把 `SheetBindings` / `SheetFieldMappings` 串到其他 Excel 文件
