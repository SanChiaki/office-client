...
当前 `_Settings` 的展示布局是单个 sheet 内上下两个可读区域：
...
当前不会再使用旧的“首列表名 + 每行一条压平记录”格式；一旦发生 metadata 写入，插件会按上述可读布局整表重写 `_Settings`。
...
| Sheet1 | owner_name | single | owner_name | false | 负责人 | 负责人 |  |  |  |  |  |  |
...
- `_Settings` 读取当前使用 `UsedRange.Value2` 批量读，不再逐单元格 COM 扫描
- `SheetBindings` 和 `SheetFieldMappings` 在一次工作簿会话内按表级做内存缓存，写入后同步刷新缓存
- 如果用户手工编辑 `_Settings`，插件会在 `_Settings` 的 `SheetChange` 事件上自动失效上述缓存；下一次业务 sheet 切换或重新触发同步动作时，会重新读取最新元数据
...
- Ribbon 当前项目状态按“活动 sheet 变化”刷新，不再随同一 sheet 内的每次选区移动重复读取 `_Settings`

一个重要细节：

- 重新选择项目时会保留当前 sheet 已维护的 `HeaderStartRow`、`HeaderRowCount`、`DataStartRow`
- 选择项目不会激活 `_Settings`
...
选择项目后，插件会自动尝试初始化当前 sheet：

- 如果当前 sheet 没有绑定
- 或绑定项目变了
- 或 `SheetFieldMappings` 为空

则自动重新初始化。

如果自动初始化失败，控制器会弹出警告，引导用户使用 `初始化当前表`。
