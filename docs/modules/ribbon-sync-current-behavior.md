...
- 模板 / `Template`
  - `应用模板` / `Apply template`
  - `保存模板` / `Save template`
  - `另存模板` / `Save as template`
- 下载 / `Download`
  - `部分下载` / `Partial download`
- 上传 / `Upload`
  - `部分上传` / `Partial upload`

Ribbon 分组、按钮和项目下拉框状态文案都会跟随当前宿主 UI 语言切换。例如：

- 中文 Excel：`项目`、`模板`、`下载`、`上传`、`账号`、`先选择项目`、`请先登录`
- 英文 Excel：`Project`、`Template`、`Download`、`Upload`、`Account`、`Select project`、`Sign in first`
